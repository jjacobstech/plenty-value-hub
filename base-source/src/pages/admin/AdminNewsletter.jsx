...
const QUILL_FORMATS = [
  ...
  'link', 'image', 'video',
  'font', 'size', 'align', 'color', 'background',
];
...
export default function AdminNewsletter() {
  ...
  const handleSend = async () => {
    ...
    setIsSending(true);
    const subscribers = await base44.entities.NewsletterSubscriber.filter({ status: 'active' }, null, 1000);
    ...
    toast({ title: 'Newsletter sent!', description: `"${subject}" has been sent to ${subscribers.length} subscribers.` });
    setIsSending(false);
  };
    ...
    <div className="min-h-full bg-gray-50">
      <div className="max-w-screen-2xl mx-auto px-4 py-6">
        ...
        <div className="mb-5 flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-2xl font-bold" style={{ color: '#001845' }}>Newsletter Composer</h1>
            <p className="text-sm mt-1 text-gray-500">Craft and send newsletters to your subscribers</p>
          </div>
          <button
        ...
        {showTemplates && (
          <div className="mb-5">
            <NewsletterTemplates onApply={handleApplyTemplate} onClose={() => setShowTemplates(false)} />
          </div>
        )}
        ...
        <div className="flex gap-6 items-start">
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-4">
            ...
            {/* Editor card */}
            <div className="rounded-2xl overflow-hidden flex flex-col bg-white" style={{ border: '1px solid #e5e7eb' }}>
              <NewsletterToolbar onImageInsert={handleImageInsert} onVideoInsert={handleVideoInsert} />

              {/* Thumbnail upload strip inside editor */}
              <div className="px-5 py-3 flex items-center gap-3" style={{ borderBottom: '1px solid #f3f4f6' }}>
                <span className="text-xs font-medium text-gray-400 uppercase tracking-wide">Thumbnail</span>
                {thumbnailPreview ? (
                  <div className="flex items-center gap-2">
                    <img src={thumbnailPreview} alt="thumb" className="w-16 h-10 rounded-lg object-cover border border-gray-200" />
                    <button
                  ...
                  <button
                    ...
                    onMouseEnter={(e) => { e.currentTarget.style.borderColor = '#81C14B'; e.currentTarget.style.color = '#81C14B'; }}
                ...
                <input ref={editorThumbnailInputRef} type="file" accept="image/*" className="hidden" onChange={handleThumbnailUpload} />
              </div>
