...
import { Users, UserCheck, UserX, TrendingUp, Download, Search, Mail, BarChart3 } from 'lucide-react';
...
export default function AdminSubscribers() {
  ...
  const exportCSV = () => {
    ...
    filtered.forEach(s => {
      rows.push([s.name || '', s.email, s.status, s.source || '', s.created_date?.split('T')[0] || '']);
    });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: '#001845' }}>Subscriber Management</h1>
          <p className="text-sm text-gray-500 mt-1">Track, manage and analyze your subscriber base</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatsCard title="Growth Rate" value={subscribers.length > 0 ? `${((active.length / subscribers.length) * 100).toFixed(0)}%` : '0%'} icon={TrendingUp} />
      </div>
              ...
              <div className="h-52">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={growthData}>
                    ...
                    <Tooltip />
                    <Line type="monotone" dataKey="count" stroke="#81C14B" strokeWidth={2} dot={{ fill: '#81C14B' }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            ) : (
              <div className="h-52 flex items-center justify-center text-muted-foreground text-sm">No growth data yet</div>
            )}
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Unsub Rate', value: inactive.length > 0 ? `${((inactive.length/subscribers.length)*100).toFixed(1)}%` : '0%' },
        ].map(m => (
          <Card key={m.label}>
            <CardContent className="p-4 flex flex-col gap-1">
              <span className="text-xs text-muted-foreground">{m.label}</span>
              <span className="text-2xl font-bold" style={{ color: '#001845' }}>{m.value}</span>
            </CardContent>
          </Card>
        ))}
      </div>
                  ...
                  {filtered.slice(0, 50).map(s => (
                    <tr key={s.id} className="border-b border-gray-50 hover:bg-gray-50">
                      ...
                      <td className="py-2.5 px-3">
                        <Badge className={s.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}>
                          {s.status}
                        </Badge>
                      </td>
                      <td className="py-2.5 px-3 text-gray-500">{s.source || '—'}</td>
                      <td className="py-2.5 px-3 text-gray-500">{s.created_date?.split('T')[0] || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              {filtered.length > 50 && (
                <p className="text-center text-xs text-gray-400 mt-3">Showing 50 of {filtered.length}</p>
              )}
            </div>
          )}
        </CardContent>
