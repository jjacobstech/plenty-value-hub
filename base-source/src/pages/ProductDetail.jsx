...
import { Star, TrendingUp, ArrowLeft, ShoppingCart, Link2, Shield, Package, BarChart3, ChevronRight } from 'lucide-react';
...
export default function ProductDetail() {
  ...
  const { data: product, isLoading } = useQuery({
    queryKey: ['product', id],
    queryFn: async () => {
      const products = await base44.entities.Product.filter({ id }, null, 1);
      return products[0];
    },
  });
  ...
  const { data: reviews = [] } = useQuery({
    ...
    queryFn: () => base44.entities.Review.filter({ product_id: id, status: 'approved' }, '-created_date', 20),
    enabled: !!id,
  });
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="space-y-6">
          ...
          {isAffiliate && (
            <div>
              ...
              <p className="text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wide">Be an Affiliate for this product</p>
      ...
      <Tabs defaultValue="description" className="mt-12">
        <TabsList className="bg-muted/50">
          ...
          <TabsTrigger value="affiliate">Become An Affiliate</TabsTrigger>
