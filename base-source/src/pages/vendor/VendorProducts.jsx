...
export default function VendorProducts() {
  ...
  const saveMutation = useMutation({
    mutationFn: async (data) => {
      ...
      if (editId) return base44.entities.Product.update(editId, payload);
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Package className="w-6 h-6 text-[#F4A300]" />
            My Products
          </h1>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={e => { e.preventDefault(); saveMutation.mutate(form); }} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <Label>Price (₦) *</Label>
                <Input type="number" step="100" min="0" value={form.price} onChange={e => setForm({ ...form, price: e.target.value })} required placeholder="e.g. 25000" />
              </div>
              <div>
                <Label>Sale Price (₦)</Label>
                <Input type="number" step="100" min="0" value={form.sale_price} onChange={e => setForm({ ...form, sale_price: e.target.value })} placeholder="Optional" />
              </div>
              <div>
