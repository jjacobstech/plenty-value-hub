...
const CAT_LABELS = {
  ...
  education: 'Education', technology: 'Technology', ai_tools: 'AI Tools',
  fashion: 'Fashion', beauty: 'Beauty', home_garden: 'Home & Garden',
  lifestyle: 'Lifestyle', ecommerce: 'E-Commerce', finance: 'Finance',
  productivity: 'Productivity', digital_services: 'Digital Services',
};
...
export default function AffiliateProducts() {
  ...
  const { data: products = [] } = useQuery({
    ...
    queryFn: () => base44.entities.Product.filter({ status: 'approved' }, '-gravity_score', 100),
  });

  const createLinkMutation = useMutation({
    mutationFn: (product) => base44.entities.AffiliateLink.create({
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm">Browse and promote high-converting products. Earn in USD.</p>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input placeholder="Search products or sellers..." value={search} onChange={e => setSearch(e.target.value)} className="pl-10" />
        </div>
      ...
      <div className="space-y-3">
        {filtered.map(product => (
          <Card key={product.id} className="hover:border-[#81C14B]/40 hover:shadow-md transition-all duration-200">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-5 flex-wrap">
                  ...
                  <div className="text-center hidden sm:block">
                    ...
                    <p className="font-bold text-green-600">{formatNGN(commissionAmount(product))}</p>
                  </div>
                  <div className="text-center hidden sm:block">
          ...
          <div className="space-y-4">
            <div className="p-4 bg-green-50 rounded-xl border border-green-200">
              <p className="text-sm text-green-700 font-medium mb-1">
                🎉 You'll earn <strong>{formatNGN(commissionAmount(selectedProduct || {}))}</strong> per sale ({selectedProduct?.commission_rate}% commission)
              </p>
              <p className="text-xs text-green-600">Share your link and start earning commissions today!</p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground mb-2 font-medium">Your tracking link:</p>
              <div className="flex items-center gap-2">
                ...
                <Button
                  size="icon"
                  variant="outline"
                  onClick={() => handleCopy(`${window.location.origin}/ref/${selectedProduct?.link?.link_code}`)}
                >
            ...
            <p className="text-xs text-muted-foreground">
              Promote via social media, WhatsApp, email, or your website. Every conversion earns you a commission!
            </p>
          </div>
        </DialogContent>
      </Dialog>
