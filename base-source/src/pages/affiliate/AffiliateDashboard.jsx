...
export default function AffiliateDashboard() {
  ...
  const { data: orders = [] } = useQuery({
    ...
    queryFn: () => base44.entities.Order.filter({ affiliate_id: user.id }, '-created_date', 50),
    enabled: !!user,
  });
      ...
      <div className="grid lg:grid-cols-3 gap-4">
                ...
                <AreaChart data={last7}>
                  <defs>
                    <linearGradient id="earn" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="#81C14B" stopOpacity={0.35} />
                  ...
                  <Area type="monotone" dataKey="earnings" stroke="#81C14B" fill="url(#earn)" strokeWidth={2} name="Earnings" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between p-3 rounded-xl" style={{ background: 'rgba(129,193,75,0.1)' }}>
              <div className="flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4" style={{ color: '#81C14B' }} />
                <span className="text-sm font-medium" style={{ color: '#81C14B' }}>Confirmed</span>
              </div>
              <span className="font-bold" style={{ color: '#81C14B' }}>{formatNGN(totalEarnings)}</span>
            </div>
              ...
              <div className="h-48">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={topLinks.map(l => ({ name: (l.product_name || 'Product').substring(0, 14), earned: l.commission_earned || 0 }))}>
                    ...
                    <Tooltip formatter={v => formatNGN(v)} />
                    <Bar dataKey="earned" fill="#81C14B" radius={[4, 4, 0, 0]} name="Earned ($)" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
                ...
                <TableBody>
                  {links.slice(0, 6).map(link => (
                    <TableRow key={link.id}>
                      ...
                      <TableCell className="font-semibold text-xs" style={{ color: '#81C14B' }}>{formatNGN(link.commission_earned)}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
