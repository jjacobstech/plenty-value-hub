...
const fileIcon = (type) => {
  if (type === 'Image' || type === 'Thumbnail') return <FileImage className="w-4 h-4" style={{ color: '#81C14B' }} />;
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-2xl p-4 bg-white" style={{ border: '1px solid #e5e7eb' }}>
        <h3 className="text-sm font-semibold mb-3" style={{ color: '#001845' }}>Thumbnail</h3>
        ...
        {thumbnailPreview ? (
          ...
          <button
            ...
            onMouseEnter={(e) => { e.currentTarget.style.borderColor = '#81C14B'; e.currentTarget.style.background = 'rgba(129,193,75,0.05)'; }}
            onMouseLeave={(e) => { e.currentTarget.style.borderColor = '#d1d5db'; e.currentTarget.style.background = '#f9fafb'; }}
          >
            <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: 'rgba(129,193,75,0.12)' }}>
        ...
        <input ref={thumbnailInputRef} type="file" accept="image/*" className="hidden" onChange={onThumbnailUpload} />
      </div>

      {/* Media Files List */}
      <div className="rounded-2xl p-4 bg-white" style={{ border: '1px solid #e5e7eb' }}>
        <h3 className="text-sm font-semibold mb-3" style={{ color: '#001845' }}>Attached Media</h3>
        {mediaFiles.length === 0 ? (
          <p className="text-xs py-4 text-center text-gray-400">No media attached yet</p>
        ) : (
          <div className="flex flex-col gap-2">
            {mediaFiles.map((file, i) => (
              <div key={i} className="flex items-center gap-3 rounded-lg px-3 py-2 bg-gray-50" style={{ border: '1px solid #e5e7eb' }}>
                {fileIcon(file.type)}
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium truncate" style={{ color: '#001845' }}>{file.name}</p>
      ...
      <div className="rounded-2xl p-4 bg-white" style={{ border: '1px solid #e5e7eb' }}>
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-sm font-semibold" style={{ color: '#001845' }}>Writing Tips</h3>
        </div>
        <div className="flex flex-col gap-2">
          {TIPS.map((tip, i) => (
