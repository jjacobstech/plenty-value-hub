...
import { registerStep1Validator, registerStep2Validator, registerStep3Validator, verifyOtpValidator, forgotPasswordValidator, resetPasswordValidator, loginValidator, signupValidator, } from '#validators/user';
...
export default class NewAccountController {
    ...
    async registerStep1({ request, response, session }) {
    ...
    async registerStep2({ request, response, session }) {
        const kyc = await request.validateUsing(registerStep2Validator);
        session.put('signup_kyc', kyc);
        return response.json({ success: true, nextStep: 3 });
    }
    async registerStep3({ request, response, session }) {
    ...
    async verifyOtp({ request, response, session, auth }) {
    ...
    async resendOtp({ response, session }) {
        const email = session.get('pending_verification_email');
        if (!email) {
            return response.redirect('/auth/signup');
        }
    ...
    async store({ request, response, session }) {
        const { role, passwordConfirmation: _passwordConfirmation, ...payload } = await request.validateUsing(signupValidator);
