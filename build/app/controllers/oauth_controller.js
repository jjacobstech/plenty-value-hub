...
const VALID_ROLES = ['vendor', 'affiliate'];
export default class OauthController {
    async redirectToGoogle({ ally, request, session }) {
        const role = request.input('role');
        const source = request.input('source', role ? 'signup' : 'login');
        session.put('oauth_source', source);
        if (VALID_ROLES.includes(role)) {
            session.put('oauth_role', role);
        }
        return ally.use('google').redirect();
    }
    async handleGoogleCallback({ ally, auth, response, session }) {
        ...
        try {
            ...
            if (!user) {
                ...
                const role = session.get('oauth_role') ?? 'affiliate';
