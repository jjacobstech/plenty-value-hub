...
export default class OrdersController {
    ...
    async processOrder({ request, auth, response }) {
        const user = auth.use('web').user;
        ...
        const salePrice = product.salePrice && new Decimal(product.salePrice).lessThan(product.price)
            ...
            : productPrice;
        const { platformFee, commissionAmount, vendorPayout } = RevenueService.calculate(productPrice, salePrice, Number.parseFloat(product.commissionRate), !!payload.affiliateLinkCode);
    ...
    async updateStatus({ params, request, auth, response }) {
        const user = auth.use('web').user;
        if (user.role !== 'admin') {
        ...
        if (['completed', 'refunded'].includes(payload.status)) {
            const emailTemplate = payload.status === 'completed' ? 'emails/order_confirmation' : 'emails/refund_notification';
            await mail.send((message) => {
                message
                    .to(order.buyerEmail)
                    .subject(payload.status === 'completed'
