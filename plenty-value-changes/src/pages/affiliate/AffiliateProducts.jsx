...
export default function AffiliateProducts() {
  ...
  const createLinkMutation = useMutation({
    mutationFn: (product) =>
      base44.entities.AffiliateLink.create({
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      result = result.filter(
        (p) => p.name?.toLowerCase().includes(q) || p.vendor_name?.toLowerCase().includes(q)
      )
    }
    ...
    if (sortBy === 'earnings')
      result.sort((a, b) => (b.avg_earnings_per_sale || 0) - (a.avg_earnings_per_sale || 0))
    return result
          ...
          <div className="space-y-4">
            <div className="p-4 bg-green-50 rounded-xl border border-green-200">
              <p className="text-sm text-green-700 font-medium mb-1">
                🎉 You'll earn <strong>{formatNGN(commissionAmount(selectedProduct || {}))}</strong>{' '}
                per sale ({selectedProduct?.commission_rate}% commission)
              </p>
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  variant="outline"
                  onClick={() =>
                    handleCopy(`${window.location.origin}/ref/${selectedProduct?.link?.link_code}`)
                  }
                >
                  {copied ? (
            ...
            <p className="text-xs text-muted-foreground">
              Promote via social media, WhatsApp, email, or your website. Every conversion earns you
              a commission!
            </p>
          </div>
        </DialogContent>
