...
const VALUE_PROPS = [
  {
    ...
    desc: 'Thorough, unbiased evaluations of the hottest products on the market.',
  },
  {
    icon: Target,
    title: 'Smart Buying Guides',
    desc: 'Category-specific guides to help you find exactly what you need.',
  },
  {
    icon: Award,
    title: 'Expert Comparisons',
    desc: 'Side-by-side product comparisons so you always get the best value.',
  },
  {
    icon: MailOpen,
    title: 'Consumer Insights',
    desc: 'Real data and trends helping 11,000+ subscribers make informed decisions.',
  },
]
...
export default function Reviews() {
  const { data: reviews = [], isLoading } = useQuery({
    ...
    queryFn: () => base44.entities.Review.filter({ status: 'approved' }, '-created_date', 50),
  })
    ...
    <div className="min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
          <div className="flex flex-wrap gap-8 items-center justify-center md:justify-start">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-sm" style={{ color: '#001845' }}>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="flex items-center justify-between mb-6">
          <h2 className="font-display text-2xl font-bold" style={{ color: '#001845' }}>
            Product Reviews
          </h2>
        ...
        <div className="space-y-4">
          {isLoading ? (
            ...
            reviews.map((review) => (
              <Card
                key={review.id}
                className="hover:shadow-md transition-all duration-200 hover:border-[#81C14B]/30"
              >
                <CardContent className="p-6">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1 mt-1">
                        {[1, 2, 3, 4, 5].map((s) => (
                          <Star
                            key={s}
                            className={`w-3.5 h-3.5 ${s <= review.rating ? 'fill-yellow-400 text-yellow-400' : 'text-border'}`}
                          />
                        ))}
            ...
            <div className="text-center py-20">
              ...
              <Link to="/marketplace" className="mt-4 inline-block">
                <Button style={{ backgroundColor: '#001845', color: '#fff' }}>
                  Browse Products
                </Button>
              </Link>
            </div>
