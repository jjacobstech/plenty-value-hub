...
export default function AdminEmailCampaigns() {
  ...
  const handleSend = async (c) => {
    await base44.entities.EmailCampaign.update(c.id, {
      status: 'sent',
  ...
  const totalRecipients = campaigns
    ...
    .reduce((s, c) => s + (c.recipients_count || 0), 0)
  const avgOpenRate =
    campaigns.filter((c) => c.recipients_count > 0).length > 0
      ? (
          (campaigns.reduce(
            (s, c) => s + (c.recipients_count > 0 ? (c.open_count || 0) / c.recipients_count : 0),
            0
          ) /
            campaigns.length) *
          100
