...
export default function AffiliateEarnings(props: AffiliateEarningsProps) {
  ...
  const totalEarnings = orders.filter(o => o.status === 'completed').reduce((sum, o) => sum + (o.commissionAmount || 0), 0);
  const pendingEarnings = orders.filter(o => o.status === 'pending').reduce((sum, o) => sum + (o.commissionAmount || 0), 0);
              ...
              <TableBody>
                {orders.map(order => (
                  <TableRow key={order.id}>
                    ...
                    <TableCell className="text-green-600 font-semibold">{formatNGN(order.commissionAmount)}</TableCell>
                    <TableCell>
