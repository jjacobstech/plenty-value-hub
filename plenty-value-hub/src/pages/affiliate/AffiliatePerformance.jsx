...
export default function AffiliatePerformance() {
  ...
  const { data: orders = [] } = useQuery({
    ...
    queryFn: () => base44.entities.Order.filter({ affiliate_id: user.id }, '-created_date', 100),
    enabled: !!user,
  });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm">Track your affiliate marketing metrics in real-time</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard title="Total Earnings (₦)" value={formatNGN(totalEarnings)} icon={Wallet} color="#81C14B" sub="All commissions" />
        <KpiCard title="Total Revenue (₦)" value={formatNGN(totalRevenue)} icon={TrendingUp} color="#F4A300" sub="Referred sales" />
        <KpiCard title="Total Clicks" value={totalClicks.toLocaleString()} icon={MousePointer2} color="#715AFF" sub={`${convRate}% conv. rate`} />
        <KpiCard title="Conversions" value={totalConversions} icon={ShoppingBag} color="#001845" sub="Completed sales" />
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Active Links', value: links.filter(l => l.status === 'active').length, note: 'Promoting now' },
          { label: 'Avg Commission', value: links.length > 0 ? formatNGN(totalEarnings / links.length) : '₦0', note: 'Per link' },
        ].map((m, i) => (
          <Card key={i}>
              ...
              <div className="h-60">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData}>
                    ...
                    <Bar dataKey="earnings" fill="#715AFF" radius={[4, 4, 0, 0]} name="Earnings (₦)" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
              ...
              <div className="h-60">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData}>
                    ...
                    <Bar dataKey="conversions" fill="#81C14B" radius={[4, 4, 0, 0]} name="Conversions" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
              ...
              <TableBody>
                {topLinks.map((link, idx) => (
                  <TableRow key={link.id}>
                    ...
                    <TableCell>{link.clicks > 0 ? formatNGN((link.commission_earned || 0) / link.clicks) : '—'}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
